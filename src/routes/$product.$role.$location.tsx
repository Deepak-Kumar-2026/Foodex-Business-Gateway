...
export const Route = createFileRoute("/$product/$role/$location")({
  ...
  head: ({ loaderData, params }) => {
    ...
    const title =
      `${p.title} ${role.label} in ${l.name} | ${company.short}`;

    const description =
      `${p.title} ${role.intent} in ${l.name} — ${p.capacity}, ${p.material}. ` +
      `${company.short}, Ghaziabad: supply, installation & spares in ` +
      `${l.cities.slice(0, 2).join(", ")}.`.slice(0, 160);
            ...
            mainEntity: [
              {
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text:
                    `${company.name}, Ghaziabad (Director: ${company.director}) ` +
                    `is a ${role.intent} of the ${p.title} serving ` +
                    `${l.cities.join(", ")} and all of ${l.name}.`,
                },
              },

              {
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text:
                    `The ${p.title} is offered in ${p.capacity} with ` +
                    `${p.material} body and ${p.automation} operation. ` +
                    `Pricing depends on capacity and customisation — ` +
                    `request a quotation for ${l.name}.`,
                },
              },
      ...
      <section className="bg-primary py-10">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <p className="mt-3 inline-flex items-center gap-1.5 rounded-full accent-gradient px-2.5 py-1 text-[10px] font-bold uppercase text-accent-foreground">
            ...
            {role.label} · {location.name}
          </p>

          <h1 className="mt-3 font-display text-3xl font-extrabold text-primary-foreground sm:text-4xl">
            {product.title} {role.label} in{" "}
            {location.name}
          </h1>

          <p className="mt-2 max-w-3xl text-sm text-primary-foreground/80">
            {company.name} is a {role.intent} of the{" "}
            {product.title} ({product.capacity},{" "}
            {product.material}) for buyers across{" "}
            {location.name} —{" "}
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-md bg-accent px-3 py-1.5 text-xs font-bold text-accent-foreground"
          >
            Looking for a {product.title}{" "}
            {otherRole.intent} in {location.name}?
          </Link>
        </div>
      </section>
      ...
      <section className="section-pad">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3">
            ...
            <div className="card-surface flex h-[300px] items-center justify-center overflow-hidden p-5 sm:h-[340px] md:h-[380px]">
              <img
                src={product.image}
                alt={`${product.title} ${role.intent} in ${location.name} — ${company.name}`}
                loading="lazy"
            ...
            {/* CITIES */}
            <h2 className="mt-8 font-display text-xl font-bold">
              Cities we serve as {product.title}{" "}
              {role.intent} in {location.name}
            </h2>
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-3">
              {[
                ...
                [
                  Headphones,
                  "After-Sales Support",
                  `Spares and service support for ${location.name} buyers, direct from Ghaziabad.`,
                ],
              ].map(
                ([Icon, head, body]) => {
                  const I =
                    Icon as typeof Truck;
            ...
            <div className="mt-2 space-y-2">
              ...
              >
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {company.name}, Ghaziabad
                  (Director:{" "}
                  {company.director}) is a{" "}
                  {role.intent} of the{" "}
                  {product.title} serving{" "}
                  {location.cities.join(", ")}{" "}
                  and all of{" "}
                  {location.name}.
                </p>
              ...
              <details className="card-surface p-4">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  ...
                  {product.capacity}), material (
                  {product.material}) and
                  automation level (
                  {product.automation}).
                  Request a quotation for a{" "}
                  {location.name}-specific offer.
                </p>
              </details>
            ...
            <div className="card-surface p-5">
              ...
              <dl className="mt-3 space-y-2 text-sm">
                {[
                  ...
                  <div
                    key={key}
                    className="flex justify-between gap-3 border-b border-border pb-1.5 last:border-0"
                  >
