...
export function GetQuoteModal({ product, onClose }: { product?: string | undefined; onClose: () => void }) {
  const { t } = useLang();
  ...
  const summary = `Quotation Request%0A${form.product || "Machine enquiry"}%0AName: ${form.name}%0APhone: ${form.phone}%0ACity: ${form.city}%0ACapacity: ${form.capacity}%0ADetails: ${form.message}`;
        ...
        {sent ? (
          <div className="space-y-4 p-6">
            ...
            <div className="card-surface p-4 text-sm">
              <h3 className="mb-2 font-display font-bold">Quotation Summary</h3>
              ...
              {form.message && <p className="mt-2 text-muted-foreground">{form.message}</p>}
            </div>
            <div className="flex flex-wrap gap-2">
              <a
                href={`https://wa.me/${company.whatsapp}?text=${summary}`}
                target="_blank"
                rel="noreferrer"
              ...
              >
                <X className="h-4 w-4" /> {t("form.close")}
              </button>
            </div>
          </div>
        ) : (
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="block text-sm">
                <span className="mb-1 block font-medium">{t("form.product")}</span>
                <input className={field} value={form.product} onChange={(e) => setForm({ ...form, product: e.target.value })} />
              </label>
              <label className="block text-sm">
                <span className="mb-1 block font-medium">{t("form.capacity")}</span>
                <input className={field} placeholder="e.g. 100 kg/hr" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })} />
              </label>
            </div>
            <label className="block text-sm">
              <span className="mb-1 block font-medium">{t("form.message")}</span>
              <textarea rows={3} className={field} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
            </label>
            <div className="flex flex-wrap gap-2 pt-1">
              <button type="submit" className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary-dark">
                <Send className="h-4 w-4" /> {t("form.submit")}
              </button>
              <a
                href={waLink(`Hi ${company.name}, I need a quotation for ${form.product || "your machines"}.`)}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-md accent-gradient px-4 py-2 text-sm font-semibold text-accent-foreground"
              >
              ...
              <button type="button" onClick={onClose} className="inline-flex items-center gap-2 rounded-md border border-input px-4 py-2 text-sm font-semibold">
                <X className="h-4 w-4" /> {t("form.close")}
              </button>
            </div>
          </form>
